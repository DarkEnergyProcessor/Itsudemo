...
inline DecrypterContext* __DctxGetBest(const void* header,const char* filename,uint8_t* game_mode)
{
	if(game_mode==NULL) return NULL;

	DecrypterContext* dctx;
	try
	{
...
inline DecrypterContext* __DctxEncryptSetupWrapper(const char* filename,uint8_t game_encryption,void* header_out)
{
	switch(game_encryption)
	{
		case 1: return EN_Dctx::encrypt_setup(filename,header_out);
		case 2: return JP_Dctx::encrypt_setup(filename,header_out);
		case 3: return TW_Dctx::encrypt_setup(filename,header_out);
		case 4: return KR_Dctx::encrypt_setup(filename,header_out);
		case 5: return CN_Dctx::encrypt_setup(filename,header_out);
		default: return NULL;
	}
}
